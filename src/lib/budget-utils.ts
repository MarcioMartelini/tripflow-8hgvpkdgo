...
export const calculateBudgetData = (
  ...
  const safeItinerarios = Array.isArray(itinerarios) ? itinerarios : []

  return CATEGORIAS.map((cat) => {
    const orcamentosForCat = safeOrcamentos.filter((o) => o?.categoria === cat)
    ...
    return {
      ...
      originalPlannedValue: orcamentosForCat.reduce((sum, o) => sum + (o?.valor_planejado || 0), 0),
