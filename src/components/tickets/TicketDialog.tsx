...
export function TicketDialog({ open, onOpenChange, tripId, ticket, onSuccess }: any) {
  ...
  useEffect(() => {
    if (open) {
      ...
      setExistingFiles(
        ticket?.arquivo ? (Array.isArray(ticket.arquivo) ? ticket.arquivo : [ticket.arquivo]) : [],
      )
      setNewFiles([])
      setFilesToRemove([])
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      if (ticket?.id) await updateTicket(ticket.id, submitData)
      else await createTicket(submitData)
